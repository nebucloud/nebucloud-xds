...
pub use data_plane_api::envoy::service::route::v3::route_discovery_service_server::{
...
impl RdsService {
    ...
    /// Create a new RDS service with a shared SotW handler.
    ///
...
pub type RdsResponseStream = ReceiverStream<Result<DiscoveryResponse, Status>>;

/// Delta response stream type for RDS.
pub type RdsDeltaResponseStream = ReceiverStream<Result<DeltaDiscoveryResponse, Status>>;

#[async_trait]
impl RouteDiscoveryService for RdsService {
    type StreamRoutesStream = RdsResponseStream;
        ...
        tokio::spawn(async move {
            ...
            while let Some(result) = tokio_stream::StreamExt::next(&mut stream).await {
                match result {
                    Ok(request) => {
                        // Validate type URL
                        if !request.type_url.is_empty() && request.type_url != TypeUrl::ROUTE {
                            error!(
                                stream = %ctx.id(),
                                expected = TypeUrl::ROUTE,
                                got = %request.type_url,
                                "invalid type URL for RDS"
                            );
                            continue;
                        }
    ...
    type DeltaRoutesStream = RdsDeltaResponseStream;

    #[instrument(skip(self, request), name = "rds_delta_stream")]
    async fn delta_routes(
        ...
        info!(stream = %ctx.id(), "Delta RDS stream started");
        ...
        tokio::spawn(async move {
                ...
                match result {
                    Ok(request) => {
                        debug!(
                            ...
                            "delta RDS request received (not yet implemented)"
                        );
                    }
                    Err(e) => {
            ...
            info!(stream = %ctx.id(), "Delta RDS stream ended");
            drop(tx);
        });
